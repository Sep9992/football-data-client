...
# VERZE: SNIPER v2 (High Precision) 🎯
# Logika: 1:1 s úspěšným backtestem (ROI 17.7%)
# Filtry: Min. kurz 1.25 | Super Safe > 0.85
...
def run_pipeline():
    print("🚀 Startuji Step 4 (SNIPER v2 Logic)...")
    ...
    # 4. PREDIKCE LOOP (SNIPER v2 LOGIC)
    new_rows = []

    # --- KONFIGURACE (Zrcadlí Step 7_2) ---
    THRESH_FAVORIT = 0.55
    THRESH_SAFE = 0.75
    THRESH_SUPER = 0.85  # Zvýšeno z 0.82
    THRESH_VALUE = 0.55
    MIN_ODDS_LIMIT = 1.25  # Nový tvrdý filtr
    ...
    for idx, row in df_fixt.iterrows():
        ...
        # Pomocná pro výpočet kurzu
        prob_dc = 0

        # DOMÁCÍ
        if ph > pa:
            prob_dc = ph + pd_prob
            # Filtr nízkých kurzů (předběžný odhad 1/prob s marží 0.9)
            fair_odd = 1 / prob_dc if prob_dc > 0 else 0
            simulated_market_odd = fair_odd * 0.90  # Konzervativní odhad trhu
    ...
    df_out["model_name"] = "hybrid_sniper_v2"
...
def generate_html_report(df):
    ...
    <head>
        <meta charset="utf-8">
        <title>Sniper Predictions v5.2</title>
        <style>
    ...
    for _, row in df.iterrows():
        ...
        if row['signal_note']:
            ...
            if "💎" in row['signal_note']: color = "#8e44ad"; bg = "#f5eef8"
    ...
    report_path = os.path.join(DATA_DIR, "predictions_report_4_2_final.html")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(html)

    print(f"📄 Sniper Report v5.2: {report_path}")
